...
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, observation_space=None) -> None:
        ...
        policy_name = getattr(self.config, "POLICY_NAME", "PointNavBaselinePolicy")
        policy_fn = baseline_registry.get_policy(policy_name)

        self.actor_critic = policy_fn(
            observation_space=observation_space,
            action_space=self.envs.action_spaces[0],
            goal_sensor_uuid=self.config.TASK_CONFIG.TASK.GOAL_SENSOR_UUID,
            extra_rgb=getattr(self.config, "EXTRA_RGB", False),
        )
        self.actor_critic.to(self.device)
        # ---------- Dropout 参数一致性检查 ----------
        target_p = getattr(self.config, "MCAN_DROPOUT", 0.1)  # 如果你在 config 中用别名，也可改为 self.config.DROPOUT_R
        n_total, n_match = 0, 0
        for name, module in self.actor_critic.named_modules():
            if isinstance(module, nn.Dropout):
                n_total += 1
                logger.info(f"[DropoutCHK] {name}  p={module.p}")
                if abs(module.p - target_p) < 1e-6:
                    n_match += 1
        logger.info(f"[DropoutCHK] Total Dropout layers={n_total},  与 config 匹配={n_match}")
        # -------------------------------------------
        self.agent = PPO(
    ...
    def _collect_rollout_step(
        ...
        # ======== 输入数据校验 ==========
        assert isinstance(rollouts.observations, dict), "rollouts.observations must be dict"
        for k, v in rollouts.observations.items():
            assert hasattr(v, 'dim'), f"Observation {k} is not a tensor or does not have dim()"
            assert v.dim() >= 2, f"Observation tensor {k} dim < 2, got {v.dim()}"
        # ===============================
        pth_time = 0.0
        env_time = 0.0
        ...
        if "rgb" in batch:
            ...
            batch_size = batch["depth"].size(0)
        elif "spectrogram" in batch:
            batch_size = batch["spectrogram"].size(0)
        else:
            # fallback to any modality
            batch_size = next(iter(batch.values())).size(0)

        if recurrent_hidden_states is None:
            recurrent_hidden_states = torch.zeros(batch_size, 1, device=actions.device)
    ...
    def _eval_checkpoint(
        ...
        if self.config.DISPLAY_RESOLUTION != model_resolution:
            ...
            if 'depth' in observation_space.spaces:
                observation_space.spaces['depth'].shape = (
                    model_resolution, model_resolution, 1
                )
            if 'rgb' in observation_space.spaces:
                observation_space.spaces['rgb'].shape = (
                    model_resolution, model_resolution, 3
                )
        ...
        assert test_recurrent_hidden_states is not None, "test_recurrent_hidden_states should be initialized"
